"""ResearchRubrics evaluator (adapted from AggAgent evaluation module)."""
...
class ResearchRubricsEvaluator(Evaluator):
    ...
    def build_prompt(self, prediction: str, item: Dict, rubric_item: Dict = None) -> str:
        """Build user prompt for a single rubric item."""
        rubric_item = rubric_item or {}
    ...
    async def _evaluate_single(self, rubric: Dict, content: str, llm: str) -> RubricResult:
        """Evaluate one rubric item against content."""
    ...
    async def compute_score(
        self, prediction: str, item: Dict, llm: str = "gpt-4.1-mini",
        error: bool = False, err_msg: str = ""
    ) -> Dict:
        if error:
